...
static float newdistance(void);  //Calculate the distance between the current position andre the waypoint
static float distance_point(float,float,float,float);  
static void put_parameter(void);
static void do_writebuf1(void);
static void do_writebuf2(void);
static void cal_mag(void);
static float CalAtanVector(void); //Calculate the inverse tangent of vectorLongi/vectorLatti; returns a value between 0~PI
static void LimitAccel(float *);
static void LimitCourseAngle(float);

static float CalFxWidth(void);
static float CalailWidth(void);
static float CalelevWidth(void);
static float CalthrotWidth(void);

static void rdTWIad(void);

static char PointArrive(void);
static void doCal(void);
static void doCapture(void);

static void resetSettingsToDefault(void);
static void readChannels(void);
static void adjustJoystickChannels(void);
static void refineJoystickChannels(void);

extern float Butterworth(float,float *);
extern void cal50(void);
...
unsigned char Parameter[100];   //settings
...
char sbus_select = 0;  //receiver type
...
char adjust_throt = 0;  //channle ajustment flag
int count_adjust = 0;   //channle ajustment stopwatch
...
int wifi_data = 0;  //wifi timeout timer
int zlf_data = 0;
char previous_x5=1;
...
int Channel_6 = 1000, Channel_Rudder = 1500, Channel_Aileron = 1500, Channel_Elevator = 1500, fpgain7 = 1500, fpgain8 = 1500, fpgain9 = 1000;
int tChannel_5 = 1000, tChannel_Rudder = 1500, tChannel_Aileron = 1500, tChannel_Elevator = 1500, tChannel_Throttle = 1930, tfpgain7 = 1500, tfpgain8 = 1500;
...
int ibus;
float fbus1=1024, fbus2=1024, fbus3=1024, fbus4=1024, fbus5=1024, fbus6=1024, fbus7=1024, fbus8=1024, fbus9=1024;
...
short  Rudder_wifi=1500,Aileron_wifi=1500,Elevator_wifi=1500,Throttle_wifi=1550, chan5Wifi = 1200, chan6Wifi = 1200;

short Rudder_MID = 1500;   //rudder channel default middle value
...
void AT91F_ST_HANDLER(void)
{
	...
	if(AT91C_BASE_DBGU->DBGU_CSR & 0x10 )
	{
        ...
        while (wifi_end - wifi_index >= 30)
        {
            isValidCmd = YES;
            
            //Reset default settings
            if ((pwifi_buffer[wifi_index] == '$') && (pwifi_buffer[wifi_index + 1] == 'H') && (pwifi_buffer[wifi_index + 2] == 'F'))
            {
            ...
            //Touch Control Channels Data
            else if ((pwifi_buffer[wifi_index] == '$') && (pwifi_buffer[wifi_index+1] == 'I') && (pwifi_buffer[wifi_index+2] == 'E'))
            {
                ...
                if (
                    (pwifi_buffer[wifi_index + 5]  ==  pwifi_buffer[wifi_index + 17]) && (pwifi_buffer[wifi_index + 6]  ==  pwifi_buffer[wifi_index + 18])
                    && (pwifi_buffer[wifi_index + 7]  ==  pwifi_buffer[wifi_index + 19]) && (pwifi_buffer[wifi_index + 8] == pwifi_buffer[wifi_index + 20])
                    && (pwifi_buffer[wifi_index + 9] == pwifi_buffer[wifi_index + 21]) && (pwifi_buffer[wifi_index + 10] == pwifi_buffer[wifi_index + 22])
                    && (pwifi_buffer[wifi_index + 11] == pwifi_buffer[wifi_index + 23]) && (pwifi_buffer[wifi_index + 12] == pwifi_buffer[wifi_index + 24])
                    && (pwifi_buffer[wifi_index + 13] == pwifi_buffer[wifi_index + 25]) && (pwifi_buffer[wifi_index + 14] == pwifi_buffer[wifi_index + 26])
                    && (pwifi_buffer[wifi_index + 15] == pwifi_buffer[wifi_index + 27]) && (pwifi_buffer[wifi_index + 16] == pwifi_buffer[wifi_index + 28])
                    )
                {
                    wifi_data = hz50;
                    ...
                    chan5Wifi = pwifi_buffer[wifi_index + 13] * 256 + pwifi_buffer[wifi_index+14];
                    chan6Wifi = pwifi_buffer[wifi_index + 15] * 256 + pwifi_buffer[wifi_index+16];
                    
                    if (Throttle_wifi > 1899) Throttle_wifi = 1899;
                }
            }
            //Data sent through Wifi
            else if ((pwifi_buffer[wifi_index] == '$') && (pwifi_buffer[wifi_index+1] == 'W') && (pwifi_buffer[wifi_index+2] == 'I'))
            {
                //Point and fly
                if (pwifi_buffer[wifi_index + 5] == 81)
                {
                ...
                else if(isSimpleCmd(CMD_SIMPLE_AIRLINE_DOWNLOAD))
                {
                    haveSendBack_all=0;
                    ctindex = 0;
                }
                         ...
                         && (pwifi_buffer[wifi_index + 7] == pwifi_buffer[wifi_index + 8]) && (pwifi_buffer[wifi_index + 9] == 84))
                {
                    ...
                    if (Waypoint_number > 100) //the maximum number of waypoints allowed is 8. but this can be changed to one's preferences.
                        Waypoint_number = 100;
                ...
                else if(isSimpleCmd(CMD_SIMPLE_POINT_FLIGHT_CANCEL))
                {
                    gimbal_target_lock = 0;
                    ClickSetTgtFlag = 0;
                }
                //Set current position as "home"
                else if(isSimpleCmd(CMD_SIMPLE_HOME_SET))
                {
                    homelatti = CurLatti;
                    homelongi = CurLongi;
                }
                //Autonomosly go "home" and land
                else if(isSimpleCmd(CMD_SIMPLE_BACK_AND_LANDING))
                {
                    //If localized
                ...
                //Auto take-off
                else if(isSimpleCmd(CMD_SIMPLE_TAKE_OFF))
                {
                ...
                //Enable RC
                else if(isSimpleCmd(CMD_SIMPLE_PHYSICAL_RC_ENABLE))
                {
                    if(rc_enabled!= 1)
                        rc_enabled = 1;
                }
                //Disable RC
                else if(isSimpleCmd(CMD_SIMPLE_PHYSICAL_RC_DISABLE))
                {
                    if(rc_enabled!=0)
                        rc_enabled = 0;
                }
                ...
                //Clear gyro
                else if(isSimpleCmd(CMD_SIMPLE_GYRO_CLEAR))
                {
                ...
                //Calibrate the R/C
                else if(isSimpleCmd(CMD_SIMPLE_CHANNEL_ADJUST))
                {   
                    adjust_throt = 1;
                ...
                // {
                else if(isSimpleCmd(CMD_SIMPLE_SETUP_ENTER)){
                ...
                //Exit configuration
                else if(isSimpleCmd(CMD_SIMPLE_SETUP_QUIT))
                {
                    if (enter_set == 1)
                    {
                        enter_set = 0;
                ...
                else if(isSimpleCmd(CMD_SIMPLE_AIRLINE_ENABLE))
                {
                    bReachTgt = 0;
                    tgtLatti = tgt_Latti[nTrkPt];
                    tgtLongi = tgt_Longi[nTrkPt];
                ...
                else if(isSimpleCmd(CMD_SIMPLE_AIRLINE_DISABLE))
                {
                    if (Enable_Waypoint == 1)
                    {
                        Enable_Waypoint = 0;
                ...
                //Close motors
                else if(isSimpleCmd(124))
                {
                    close = 1;
                }
                //Open throttle lock
                else if(isSimpleCmd(CMD_SIMPLE_MOTRO_UNLOCK))
                {
                ...
                else if(isSimpleCmd(143))
                {
                    send_limited = 1;
                }
                //Start carefree
                else if(isSimpleCmd(CMD_SIMPLE_CAREFREE_ENABLE))
                {
                ...
                //Exit Carefree
                else if(isSimpleCmd(CMD_SIMPLE_CAREFREEE_DISABLE))
                {
                    Waypoint_carefree = 0;
                    carefree = 0;
                }
                //Send compass data to GCS
                else if(isSimpleCmd(CMD_SIMPLE_MAG_DATA_DOWNLOAD))
                {
...
void IO_init(void)
{
	...
	AT91F_PMC_EnablePeriphClock(
                                AT91C_BASE_PMC, //pointer to PMC controller
                                (((unsigned int) 1 << AT91C_ID_PIOC) |
                                 ((unsigned int) 1 << AT91C_ID_PIOB) |
                                 ((unsigned int) 1 << AT91C_PB29_IRQ0) |
                                 ((unsigned int) 1 << AT91C_ID_PIOA)) );  //IDs of peripherals to enable
    
	AT91C_BASE_PIOB->PIO_ODR =1 << 29;
}


void adjustJoystickChannels(void)
{
    count_adjust++;
...
void refineJoystickChannels(void){
    //1100~1930		7~90
...
void readChannels(void)
{
    ...
    if (!rc_enabled && (wifi_data != 0)) {  //Touch Control Mode,and WiFi is connected to GCS
        tChannel_5 = chan5Wifi;
        Channel_5 = tChannel_5;
        
        yChannel_6 = chan6Wifi;
        Channel_6 = yChannel_6;
        ...
        //1100~1930		7~90
        tChannel_Throttle = (float)(tChannel_Throttle - 1000) * (900 / (float)(2000 - 1000)) + 1100;
        tChannel_Rudder   = (float)(tChannel_Rudder - 1000) * (800 / (float)(2000 - 1000)) + 1100;
        tChannel_Aileron  = (float)(tChannel_Aileron - 1000) * (800 / (float)(2000 - 1000)) + 1100;
        tChannel_Elevator = (float)(tChannel_Elevator - 1000) * (800 / (float)(2000 - 1000)) + 1100;
    }
    else{ //Physical Control Mode, or WiFi is not connected to GCS
        if(rc_enabled!= 1)
            rc_enabled = 1;
        
        if ((run_time < 3) && (sbus_select == 0))  //auto
        {
            ...
            if ((tChannel_Rudder > 1000)
                && (tChannel_Rudder < 2000)
                && (tChannel_Aileron > 1000)
                && (tChannel_Aileron < 2000)
                && (tChannel_Elevator > 1000)
                && (tChannel_Elevator < 2000)
                && (tChannel_Throttle > 700)
                && (tChannel_Throttle < 2300)){
                ppmmode = 1;
            }
            else
                ppmmode = 0;
        ...
        if (((ppmmode == 1) && (sbus_select == 0)) || (sbus_select == 3)) //ppm
        {
            ...
            if (adjust_throt)
            {
                adjustJoystickChannels();
            }
            else
            {
                refineJoystickChannels();
            }
        ...
        else if (((sbus_error == 1) || ( sbus_select == 1)) && (sbus_select != 2)) //standard
        {
            tChannel_5 = (((unsigned char)*pmem_fpga1h) << 8) + (unsigned char)*pmem_fpga1l;
            Channel_5 = tChannel_5;
            
            tChannel_Rudder   = (((unsigned char)*pmem_rudderh) << 8) + (unsigned char)*pmem_rudderl;
            tChannel_Aileron  = (((unsigned char)*pmem_aileronh) << 8) + (unsigned char)*pmem_aileronl;
            tChannel_Elevator = (((unsigned char)*pmem_elevatorh) << 8) + (unsigned char)*pmem_elevatorl;
...
void AT91F_IRQ0_HANDLER(void)		
{
	...
	if (errorflash == 1)
        return;		//Error while reading flash. Needs to be restarted.
	if (endinit == 0)
        return;		//Initiailization isn't complete yet.
    ...
    *pmem_maggo = 3;
    ...
    if (ctreceiver >= hz250/25)
    {
        ...
        {
            ctgohome++;
            if (ctgohome > 5 * 50)									//Only after 5 seconds, it starts going back
            {
                ctgohome = 5 * 50;
                Channel_6 = 2000;
    ...
    else if (DroneMode == 8)
    {
        ...
        else if (versionX  ==  20)			//X8，共轴，4轴8电机
        {
            //X8
            cal1 += nCalFxWidth;		//正的，往顺时针转
            cal2 += nCalFxWidth;
    ...
    *pmem_fpga8h = tmph;
    ...
    if (Parameter[GIMBAL_ROLL_SENSITIVITY] > 128)
        y13 = (float)Parameter[GIMBAL_ROLL_SENSITIVITY] - 256;
    else
        y13 = (float)Parameter[GIMBAL_ROLL_SENSITIVITY];
    ...
    if (Parameter[GIMBAL_ROLL_SENSITIVITY] == 0)
        tout = 0;
    ...
    if(AT91F_PDC_IsTxEmpty(AT91C_BASE_PDC_DBGU))
    {
        ...
        if(send_paraX == 1)
        {
            ...
            msg[3] = 'R';
            paraUSER[32] = 0x00;
            paraUSER[32]= (unsigned char)('$'+'P'+'U'+'R');
            for(i=0;i<32;i++)
            {
                msg[i+4]=paraUSER[i];
        ...
        if(rdParaHeadFlag == 1)
        {
            ...
            msg[3]='R';
            Parameter[40]=0x00;
            Parameter[40]= (unsigned char)('$'+'P'+'A'+'R');
            for(i=0;i<40;i++)
            {
                msg[i+4]=Parameter[i];
...
int main(void)
{
	...
	*pmem_rudderl = tmpl;   //rudder
	*pmem_rudderh = tmph;
    
	*pmem_aileronl = tmpl;    //aileron
	*pmem_aileronh = tmph;
    
	*pmem_elevatorl = tmpl;  //pitch
	*pmem_elevatorh = tmph;
	...
	*pmem_355_rst_ = 0x01;
	...
	AT91F_US_SetBaudrate ((AT91PS_USART)AT91C_BASE_US2, 59904000, BAUD_YS); //将串口二的波特率设为115200
	...
	*pmem_poke	= 81;
	...
	while(1)	//main loop
	{
		if (Parameter[STATE_SENSITIVITY] < 50)
            Parameter[STATE_SENSITIVITY] = 50;
		if (Parameter[STATE_SENSITIVITY] > 80)
            Parameter[STATE_SENSITIVITY] = 80;
		...
		*pmem_width2l = tmpll;
		...
		SndBuf[CURRENT_PROD_NUMBER_2] = (int)(VERSION / 256);
		SndBuf[CURRENT_PROD_NUMBER_1] = (int)(VERSION - SndBuf[CURRENT_PROD_NUMBER_2] * 256);
...
float CalthrotWidth(void)
{
	...
	else					//Completely manual, increase throttle support
	{
		...
		else
		{
			...
			if (nCalthrotWidth < 1100 + maxim)
                nCalthrotWidth = 1100 + maxim;
		}
	}
...
{
	float distance, sin_t, cos_t;
	distance = sqrt( ((double)tgtLatti - (double)tgtLatti_s) * ((double)tgtLatti - (double)tgtLatti_s) + ((double)tgtLongi - (double)tgtLongi_s) * ((double)tgtLongi - (double)tgtLongi_s) * (double)COS_FACTOR * (double)COS_FACTOR );
	if (distance != 0)
	{
		sin_t = (tgtLatti - tgtLatti_s) / distance;
		cos_t = (tgtLongi - tgtLongi_s) * COS_FACTOR / distance;
	}
	else
	{
		sin_t = 0;
	...
	distance = (CurLongi - tgtLongi) * sin_t * COS_FACTOR + (tgtLatti - CurLatti) * cos_t;
	distance = fabsf(distance * 110000);
	...
	for (loop = 0; loop < 2; loop++)
	{
		if (!(iic_rdbuf[loop * 2] & 0x80))
		{
			...
			switch (which)
			{
                ...
                {
                    if (loop == 0) {
                        v12U = ((unsigned char)iic_rdbuf[loop * 2 + 1] + ((unsigned char)iic_rdbuf[loop * 2] & 0x0f) * 256);
                    }
                    break;
                }
                case 3:
                {
                    if (loop == 1)
                    {
                        vFORCEU = ((unsigned char)iic_rdbuf[loop * 2 + 1] + ((unsigned char)iic_rdbuf[loop * 2] & 0x0f) * 256);
                    }break;
                }
			}
		}
...
//将各通道的中值和油门的最大值和最小值保存到dataflash中
...
void cal_mag()
{
	float sinGama, cosGama, cosTheta, sinTheta;
	float tzmagBias;
	float magDec;
	...
	if ((Parameter[ERASE_ROMBOOT] == 90) && (enter_set != 1))		//display kmag params
	{
		...
		tzmagBias = zmag_bias * PI / 180;
		float2buf(&OUT_INA_HANGXIANG, &tzmagBias);
	}
	...
	Gxmag = xmag * cosTheta + throtag * sinTheta * sinGama + zmag * sinTheta * cosGama;
	Gthrotag = throtag * cosGama - zmag*sinGama;
	...
	if ((unsigned char)Parameter[MAGNETIC_VARIATION] < 127)					//positive
	{
		if ((unsigned char)Parameter[MAGNETIC_VARIATION] <= 100)
			magDec =  (float)((unsigned char)Parameter[MAGNETIC_VARIATION]) / 10;
		else
			magDec = (float)((unsigned char)Parameter[MAGNETIC_VARIATION]) - 100;
	}
	else																		//negative
	{
		magDec = (float)((unsigned char)Parameter[MAGNETIC_VARIATION]-256);
		if (magDec >= -100)
			magDec = magDec / 10;
		else
			magDec = magDec + 100;
...
void LimitCourseAngle(float v)
{
	...
	deltaAngle = v - ref_yaw;
	if (deltaAngle > PI) deltaAngle -= 2 * PI;
	if (deltaAngle < -PI) deltaAngle += 2 * PI;
    
	if (fabsf(deltaAngle) <= deltaMax)
	{
		ref_yaw = v;
	}
	else
